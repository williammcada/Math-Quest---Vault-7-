...
button.addEventListener('click', async () => {
  ...
  try {
    ...
    if (catalog.version !== '0.6.1') throw new Error(`Backend is v${catalog.version || 'unknown'}. Run the v0.6.1 backend update first.`);
    ...
    if (health.service !== 'MathQuest' || health.ok !== true || health.version !== '0.6.1') throw new Error('Unexpected response from the session server.');
