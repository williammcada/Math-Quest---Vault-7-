import {WORLD,BUILDINGS,TASKS,PICKUPS,PROPS,DISTRACTIONS,doorRects,propBounds,walls,nextObjective} from './world.js?v=0.9.1';
import {drawCar,drawFixtures} from './hardware.js?v=0.9.1';
...
export function render(c,s,art,{map=s.map}={}){
 ...
 for(const p of PROPS){if(p.wire&&s.tasks.power)c.globalAlpha=.5;if(p.art<=3)drawCar(c,p,s);else cell(c,art.props,p.art,4,4,p.x,p.y,p.w,p.h);c.globalAlpha=1;if(p.burning){const glow=c.createRadialGradient(p.x+p.w/2,p.y+p.h/2,4,p.x+p.w/2,p.y+p.h/2,100);glow.addColorStop(0,'rgba(255,126,36,.28)');glow.addColorStop(1,'rgba(255,126,36,0)');c.fillStyle=glow;c.fillRect(p.x-100,p.y-100,p.w+200,p.h+200);}}
 ...
 for(const wall of walls(s).filter(w=>!w.window)){for(let x=wall.x;x<wall.x+wall.w;x+=16)for(let y=wall.y;y<wall.y+wall.h;y+=16)cell(c,art.tiles,wall.gate?12:5,4,4,x,y,16,16);}
 ...
 for(const d of doorRects()){const v=s.doors[d.id];c.fillStyle=v.hp<=0?'#69544b':v.closed?v.hp<40?'#c95748':v.hp<70?'#c99345':'#71a3ab':'#56866b';c.fillRect(d.x,d.y,d.w,v.closed?16:4);if(v.closed){c.strokeStyle='#eee';c.strokeRect(d.x,d.y,d.w*v.hp/100,3);}}
 drawFixtures(c,s);
