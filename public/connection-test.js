...
button.addEventListener('click', async () => {
  ...
  try {
    ...
    if (catalog.version !== '0.9.1') throw new Error(`Backend is v${catalog.version || 'unknown'}. Run the v0.9.1 backend update first.`);
    ...
    const response = await fetchApi(hosting.api('health'), {
      method: 'POST', headers: { 'content-type': 'application/json',authorization:'Bearer connectivity-check-not-a-credential' },
    ...
    if (health.service !== 'MathQuest' || health.ok !== true || health.version !== '0.9.1') throw new Error('Unexpected response from the session server.');
