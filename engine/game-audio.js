export class GameAudio{
 constructor(media){this.media=media;this.enabled=false;this.volume=.45;this.failed=[];this.buffers={};this.voices=new Set();}
 async start(){
   this.ctx ||=new(window.AudioContext||window.webkitAudioContext)();await this.ctx.resume();this.enabled=true;
   ...
   if(this.enabled)this.music('ambient');return this.failed;
 }
 ...
 async toggle(){if(this.enabled){this.enabled=false;await this.ctx?.suspend();return;}if(!this.ctx){await this.start();return;}this.enabled=true;await this.ctx.resume();if(!this.track)this.music('ambient');}
 pause(value){if(!this.ctx||!this.enabled)return;value?this.ctx.suspend():this.ctx.resume().catch(()=>{});}
 destroy(){this.enabled=false;this.track?.stop();for(const v of this.voices)v.stop();this.ctx?.close();}
}
