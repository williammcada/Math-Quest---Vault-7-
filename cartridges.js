...
export const NIGHTFALL = {
  id: 'nightfall', title: 'Nightfall: Last Bus Out', revision: 'nightfall-city-2',
  contract: 'mq.cartridge/1.0', gameId: 'topdown-combat', activeLimitMs: 7200000,
  teamWindowMs: null, budget: 40,
  ...
  items: [
    {id:'ammo-pouch',title:'Ammo pouch',cost:20,text:'Start with 36 rounds instead of 12. Pickups still replenish ammunition.'},
...
export function sceneFor(c, team, names, crew) {
  ...
  const scenes={
    briefing:['The Silent Terminal', `The terminal clock stopped at 11:47. Its departure board still promises buses to places that have stopped answering the telephone. Outside, the emergency shutters shudder whenever something leans against them. Inside, a single engine coughs and dies.`, `The driver introduces himself as Sol. He has counted the seats three times. He has fuel for one journey, a failing battery, and a radio that only works when somebody holds its broken wire against the dashboard. Your crew, ${roster}, is the last team still moving between buildings.`, `Two calls break through the static. A medic is trapped in the clinic. A mechanic is waiting at the depot. Both say the same thing: do not leave without us. Sol looks from the radio to the fuel gauge. He cannot promise both. Neither can you.`, 'Restore the radio, recover the battery, and get this bus running. Then your crew must decide what it owes the people who are still waiting. At the end, each of you plays an individual evacuation run: search Maintenance and Dispatch, restore the substation, open the garage, and bring a replacement battery back to the bus. You do not have to clear the city. You have to keep moving.'],
  ...
  if(team.stage==='gate'){
    ...
    const bodies={
      ...
      'Last Ignition':['The garage inventory confirms a charged replacement battery. But the electric shutter is locked down, and the terminal key hooks are empty. Sol spreads the district map across the dashboard. The escape now has a plan, not yet a working engine.','Finish the departure checks and claim forty supply credits. Then choose equipment and a destination. Each crew member will run a solo evacuation simulation: restore power, recover the keys and battery, and get the bus moving.']
