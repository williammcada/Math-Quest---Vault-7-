import {REVISION,TASKS,PICKUPS,PROPS,DISTRACTIONS,doorRects,solid,lineClear,nextObjective,seededEnemies} from './world.js';
import {threatFor} from './config.js';
...
export function restoreState(raw,loadout=[],route='clinic',threat=1){
 ...
 for(const key of ['tasks','picked','checkpoint','doors','distractions'])if(raw[key]&&typeof raw[key]==='object')s[key]=structuredClone(raw[key]);
 ...
 for(const e of s.enemies){const old=known.get(e.id);if(old&&Number.isFinite(old.x)&&Number.isFinite(old.y)){e.x=Math.max(16,Math.min(2544,old.x));e.y=Math.max(16,Math.min(1264,old.y));e.hp=Math.max(0,Math.min(e.hp,Number(old.hp)||0));e.phase=e.hp?'return':'dead';e.target={x:e.homeX,y:e.homeY};}}
...
export function damage(s){
 ...
 if(s.vest){s.vest--;s.blocks++;emit(s,'vest',`Vest blocked a strike · ${s.vest} plates remain.`);return;}
 s.health--;s.hits++;emit(s,'hurt','Hit! Break sight, close a door, or look for first aid.');
...
export function triggerDistraction(s,id){
 ...
 s.distractions[id]={started:s.time,until:s.time+d.duration};s.distractionsUsed++;makeNoise(s,d.x,d.y,d.radius,d.duration,'lure');emit(s,'power',d.kind==='barrel'?'Fuel barrel ignited! Keep clear of the flames.':'Lure active. Move away quietly while the infected investigate.');
 if(d.kind==='barrel'){for(const e of s.enemies)if(e.hp>0&&dist(e,d)<115&&lineClear(s,e,d)){e.hp=Math.max(0,e.hp-4);if(!e.hp){e.phase='dead';e.deathTime=s.time;}}if(dist(s,d)<100)damage(s);}return true;
}
export function completeTask(s,id){
 ...
 else emit(s,id==='power'?'power':'objective',({fuse:'High-current fuse recovered.',power:'Power restored. Fallen lines isolated. Garage shutter open.',keys:'Bus keys recovered.',battery:'Battery recovered. Return to the bus.',installed:'Battery installed. Bring the keys and start the engine.'})[id]);
...
export function step(s,input,dt=1/60){
 ...
 for(const d of DISTRACTIONS){const live=s.distractions[d.id];if(live&&live.until>s.time){if(s.noise<=0)makeNoise(s,d.x,d.y,d.radius,.5,'lure');if(d.kind==='barrel'){if(dist(s,d)<60)damage(s);for(const e of s.enemies)if(e.hp>0&&dist(e,d)<60){e.hp=Math.max(0,e.hp-dt*2);if(!e.hp){e.phase='dead';e.deathTime=s.time;}}}}}
 const active=s.enemies.filter(e=>e.hp>0&&dist(e,s)<650).sort((a,b)=>dist(a,s)-dist(b,s)).slice(0,threatFor(s.threat).active);s.activeCount=active.length;
 ...
 const task=TASKS.find(t=>!s.tasks[t.id]&&dist(t,s)<52),door=doorRects().find(d=>s.doors[d.id].hp>0&&dist(s,{x:Math.max(d.x,Math.min(d.x+d.w,s.x)),y:d.y+8})<48),lure=DISTRACTIONS.find(d=>d.kind!=='barrel'&&!s.distractions[d.id]&&dist(d,s)<52);
 s.nearby=task?.id||null;s.prompt=task?`Hold Search / Use: ${task.label}`:door?`Tap Search / Use: ${s.doors[door.id].closed?'open':'close'} ${door.id} door (${Math.ceil(s.doors[door.id].hp)}%)`:lure?`Tap Search / Use: ${lure.label}`:'';
