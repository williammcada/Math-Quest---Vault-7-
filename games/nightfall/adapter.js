...
export const nightfallAdapter={
 id:'nightfall-city',title:'Nightfall · Last Bus Out',revision:'nightfall-city-2',media:MEDIA,
 create:run=>restoreState(run.snapshot,run.loadout,run.route),step,render,loadArt,
 instructions:'Find the fuse in Maintenance, restore the substation, search Dispatch for keys, and bring the garage battery to the bus. The Clinic rescue is optional. Ammunition is scarce. Avoid enemies when you can.',
 ...
 hud:s=>`HEALTH ${s.health}/3   AMMO ${s.ammo}   VEST ${s.vest}   MEDKIT ${s.medkit}   DAMAGE ${s.damage}`,
 objective:s=>nextObjective(s)?.label||'Bus ready',
 ...
 assisted(s,root,onProgress){
   ...
   root.querySelector('[data-risk]').onclick=()=>{const cost=s.damage===2?2:4;if(s.ammo>=cost){s.ammo-=cost;s.shots+=cost;completeTask(s,target.id);onProgress();}else if(s.vest){s.vest--;s.blocks++;completeTask(s,target.id);onProgress();}else if(s.medkit){s.medkit--;s.heals++;completeTask(s,target.id);onProgress();}else root.querySelector('[data-feedback]').textContent='You lack ammunition and protection. Use the covered route.';};
