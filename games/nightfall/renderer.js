import {WORLD,BUILDINGS,TASKS,PICKUPS,PROPS,DISTRACTIONS,doorRects,propBounds,walls,nextObjective} from './world.js';
...
export function render(c,s,art,{map=s.map}={}){
 ...
 for(const d of DISTRACTIONS){const live=s.distractions[d.id],burn=live&&live.until>s.time;c.fillStyle=d.kind==='barrel'?'#d9763e':'#e4b958';c.beginPath();c.arc(d.x,d.y,14,0,Math.PI*2);c.fill();if(burn){c.strokeStyle='#ffb65c';c.beginPath();c.arc(d.x,d.y,30+(s.time*25)%50,0,Math.PI*2);c.stroke();}if(d.kind==='barrel'&&burn){c.fillStyle='#fc7c3e55';c.beginPath();c.arc(d.x,d.y,60,0,Math.PI*2);c.fill();}}
